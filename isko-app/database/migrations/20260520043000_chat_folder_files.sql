create table if not exists public.chat_folders (
  ...
  constraint chat_folders_title_length check (char_length(title) between 1 and 120),
  constraint chat_folders_selected_tool_allowed check (selected_tool in ('', 'Math', 'Programming', 'Complex Problems'))
);
...
create index if not exists chat_threads_folder_updated_idx
  on public.chat_threads (folder_id, updated_at desc);
